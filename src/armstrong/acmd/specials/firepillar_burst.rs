...
unsafe extern "C" fn ssbexo_armstrong_firepillar_burst_acmd(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        let charge = WorkModule::get_float(agent.module_accessor, *WEAPON_ARMSTRONG_FIREPILLAR_INSTANCE_WORK_ID_FLOAT_CHARGE);
...
unsafe extern "C" fn ssbexo_armstrong_firepillar_burst_effect(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        ...
        let effect = EffectModule::req(agent.module_accessor, Hash40::new("armstrong_ground_flame_crack"), &Vector3f{x: owner_pos_x+(pos_x*owner_lr), y: owner_pos_y+pos_y, z: owner_pos_z}, &Vector3f{x: 0.0, y: 0.0, z: 0.0}, 1.0, 0, -1, false, 0);
    ...
    if is_excute(agent) {
        ...
        let effect = EffectModule::req_on_joint(agent.module_accessor, Hash40::new("armstrong_flame_pillar"), Hash40::new("top"), &Vector3f{x: 0.0, y: 0.0, z: 0.0}, &Vector3f{x: 90.0, y: 0.0, z: 0.0}, 1.0, &Vector3f{x: 0.0, y: 0.0, z: 0.0}, &Vector3f{x: 0.0, y: 0.0, z: 0.0}, false, 0, -1, 0);
        WorkModule::set_int(agent.module_accessor, effect as i32, *WEAPON_ARMSTRONG_FIREPILLAR_INSTANCE_WORK_ID_INT_EFFECT_ID);
    }
}
